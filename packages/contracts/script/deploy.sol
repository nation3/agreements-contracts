...
contract DeployStack is Script, DeploymentUtils {
    ...
    function setUpArbitrator() internal {
        if (ARBITRATOR == address(0)) {
            address arbitrationToken = registeredContractsAddress[
                "ArbitrationToken"
            ];

            Arbitrator arbitrator = new Arbitrator{salt: SALT}(
                Permit2(PERMIT2),
                tx.origin
            );
            arbitrator.setUp(
    ...
    function setUpFramework() internal {
        ...
        CollateralAgreementFramework framework = new CollateralAgreementFramework{
                salt: SALT
            }(Permit2(PERMIT2), tx.origin);
        framework.setUp(
            arbitrator,
            DepositConfig(arbitrationToken, DISPUTE_COST, tx.origin)
        );
    ...
    function storeDeploymentManifest() internal {
        ...
        console.log(
            "Stored deployment manifest at %s.",
