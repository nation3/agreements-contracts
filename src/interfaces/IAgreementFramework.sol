...
interface IAgreementFramework is IArbitrable {
    ...
    event AgreementCreated(bytes32 indexed id, bytes32 termsHash, uint256 criteria, string metadataURI, address token);
    ...
    event AgreementPositionUpdated(bytes32 indexed id, address indexed party, uint256 balance, PositionStatus status);
