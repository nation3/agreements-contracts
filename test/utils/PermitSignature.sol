...
contract PermitSignature is Test {
    ...
    function defaultERC20PermitMultiple(TokenPair[] memory tokenPairs, uint256 nonce)
        internal
        view
        returns (ISignatureTransfer.PermitBatchTransferFrom memory)
    {
        ISignatureTransfer.TokenPermissions[] memory permitted =
            new ISignatureTransfer.TokenPermissions[](tokenPairs.length);
        for (uint256 i = 0; i < tokenPairs.length; ++i) {
            permitted[i] = ISignatureTransfer.TokenPermissions({token: tokenPairs[i].token, amount: tokenPairs[i].amount});
        }
